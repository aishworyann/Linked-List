...
void insert_last(Node **head_ref, int value) {
  ...
  last_node->next =
      new_node; // make the last node pointing to the new node we created
}
...
void delete_node(Node *head_ref, int count) {
  ...
  while (i < count - 1) { // loop will run till count -1 so that we can access
                          // the next                               element
    prev = prev->next;
    i++;
  }
...
void from_end(Node *head_ref, int n) {
...
int main() {
  ...
  push(&head,
       100); // as we are using double pointer , we need to pass the address of
             // the head pointer in the function
             // time complexity is O(1)

  // insertion of node after a node
  insertafter(
      second,
      200); // first arg is the node we want to add the node after , second is
            // the value time complexity is O(n)
